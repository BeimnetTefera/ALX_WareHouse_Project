...
SELECT
    ...
    TRIM(
    CASE 
        WHEN cp.contact_phone LIKE '+234%' THEN cp.contact_phone
        WHEN cp.contact_phone LIKE '234%'  THEN CONCAT('+', cp.contact_phone)
        WHEN cp.contact_phone LIKE '0%'    THEN CONCAT('+234', SUBSTRING(cp.contact_phone, 2, LEN(cp.contact_phone)))
        ELSE CONCAT('+234', cp.contact_phone)
    END
) AS [Phone_Number],
