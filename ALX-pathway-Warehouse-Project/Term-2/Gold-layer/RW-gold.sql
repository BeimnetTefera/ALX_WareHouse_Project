...
SELECT
    ...
    TRIM(
    CASE 
        WHEN cp.contact_phone LIKE '+250%' THEN cp.contact_phone
        WHEN cp.contact_phone LIKE '250%'  THEN CONCAT('+', cp.contact_phone)
        WHEN cp.contact_phone LIKE '0%'    THEN CONCAT('+250', SUBSTRING(cp.contact_phone, 2, LEN(cp.contact_phone)))
        ELSE CONCAT('+250', cp.contact_phone)
    END
) AS [Phone_Number],
