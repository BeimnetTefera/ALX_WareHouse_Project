...
SELECT
   ...
   TRIM(
    CASE 
        WHEN cp.contact_phone LIKE '+254%' THEN cp.contact_phone
        WHEN cp.contact_phone LIKE '254%'  THEN CONCAT('+', cp.contact_phone)
        WHEN cp.contact_phone LIKE '0%'    THEN CONCAT('+254', SUBSTRING(cp.contact_phone, 2, LEN(cp.contact_phone)))
        ELSE CONCAT('+254', cp.contact_phone)
    END
) AS [Phone_Number],
