...
SELECT
    ...
    TRIM(
    CASE 
        WHEN cp.contact_phone LIKE '+233%' THEN cp.contact_phone
        WHEN cp.contact_phone LIKE '233%'  THEN CONCAT('+', cp.contact_phone)
        WHEN cp.contact_phone LIKE '0%'    THEN CONCAT('+233', SUBSTRING(cp.contact_phone, 2, LEN(cp.contact_phone)))
        ELSE CONCAT('+233', cp.contact_phone)
    END
) AS [Phone_Number],
